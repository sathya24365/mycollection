...
sub capella{
	...
	# my $capella_cmd = "java -jar Installation.jar -f ".$location_capella."deviceProfileMapping.xml -install";
	# print "\n$capella_cmd\n";
	# system($capella_cmd);
	# print"\n";
	
	ntf_message("Capella Loading");
	my $location_capella= 'C:\QafastInstaller\\';
	my $location_capella_local= 'C:\Dropbox\Installation-bundle-5.0.3\\';
	chdir($location_capella_local) or die "cannot change: $!\n";
	my $capella_cmd = "java -jar Installation.jar -f ".$location_capella."deviceProfileMapping.xml -install";
	print "\n$capella_cmd\n";
	# system($capella_cmd);
	...
	if ( grep( /^$pmessage$/, @out ) ) {
		...
		ntf_message("Capella is installed");
	

	} else{ 
		ntf_message("Capella has been failed");
		exit;
	}
