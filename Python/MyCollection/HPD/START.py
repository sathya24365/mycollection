...
def NAT():
	...
	if int(WFC_Cmd) == 1:
		...
		while 1:
			print "NAT_Start", inc()
			os.system("start /MIN cmd /c adb -s %s shell hal_proxy_daemon wfc wlan0 %s %s [%s] %s %s %s %s %s" %(D_ID, WFC_Cmd, IP_packet_length, IP_packet_Hex, source_mac, dest_mac, period_in_msec, iterations, interval))
		...
		while 1:
			print "NAT_Stop", inc()
			os.system("start /MIN cmd /c adb -s %s shell hal_proxy_daemon wfc wlan0 %s %s %s" %(D_ID, WFC_Cmd, iterations, interval))
			time.sleep((int(interval)*int(iterations))+5)
			adb(D_ID, 'busybox killall hal_proxy_daemon')
	else:
...
def GSCAN():
	...
	while 1:
		print "GSCAN", inc()
		os.system("start /MIN cmd /c adb -s %s shell hal_proxy_daemon gscan wlan0 1 2"%(D_ID))
		time.sleep( 5 )
		adb(D_ID, 'busybox killall hal_proxy_daemon')

def RTT():

	RTT = minidom.parse("RTT.xml")

	ARG1 = RTT.getElementsByTagName("Req_ID")[0]
	ARG2 = RTT.getElementsByTagName("RTT_Cmd")[0]
	ARG3 = RTT.getElementsByTagName("iterations")[0]
	ARG4 = RTT.getElementsByTagName("interval")[0]

	RTT_Cmd=ARG2.firstChild.data
	iterations=ARG3.firstChild.data
	interval=ARG4.firstChild.data
	if int(RTT_Cmd) == 1:
		print RTT_Cmd, iterations, interval
		while 1:
			print "RTT_Request", inc()
			os.system("start /MIN cmd /c adb -s %s shell hal_proxy_daemon RTT wlan0 %s %s %s" %(D_ID, RTT_Cmd, iterations, interval))
			# os.system("echo adb shell hal_proxy_daemon RTT wlan0 %s %s %s" %(RTT_Cmd, iterations, interval))
			time.sleep((int(interval)*int(iterations))+5)
			adb(D_ID, 'busybox killall hal_proxy_daemon')
		...
		print RTT_Cmd, iterations, interval
		while 1:
			print "RTT_Cancel", inc()
			os.system("start /MIN cmd /c adb -s %s shell hal_proxy_daemon RTT wlan0 %s %s %s" %(D_ID, RTT_Cmd, iterations, interval))
			time.sleep((int(interval)*int(iterations))+5)
			adb(D_ID, 'busybox killall hal_proxy_daemon')
	else:
		print "Got a false value, Update RTT.xml with correct value"
...
def RSSI():
	...
	if int(WFC_Cmd) == 3:
		...
		while 1:
			print "RSSI_Start", inc()
			os.system("start /MIN cmd /c adb -s %s shell hal_proxy_daemon wfc wlan0 %s %s %s %s %s" %(D_ID, WFC_Cmd, Min_Rssi, Max_Rssi, iterations, interval))
