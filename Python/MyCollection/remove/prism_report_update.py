...
class Prism:
  ...
  def getChangeRequestDateFA(self, key):
    ...
    for part in response['ChangeRequest']['Participants']['ParticipantEntity']:
  ...
  def getChangeRequestDetailsDateStatus(self,key):
    ...
    for part in response['ChangeRequest']['Participants']['Participant']:
      if part['IsPrimary'] == True:
        return (response['ChangeRequest']["Status"],response['ChangeRequest']\
                ["CreatedDate"],part['Area'], part['SubSystem'], part['SwFunctionality'],\
                response['ChangeRequest']["Title"])

  def getChangeRequestFirstSR(self,key):
    request = self.client.factory.create('ns1:CRRequest')
    request['ChangeRequestId'] = key
    #print key
    response = self.client.service.GetChangeRequestById([request])
    #for part in response['ChangeRequest']['Participants']['ParticipantEntity']:
    CheckRecord = response['ChangeRequest']['CustomerRecords']
    if CheckRecord != None:
        CustRecords = response['ChangeRequest']['CustomerRecords']['CustomerRecord']
        if len(CustRecords) >0:
            return (len(CustRecords),CustRecords[0]['SRNumber'])
    else:
        return ('NoSR Attached','NoSR Attached')


  def getChangeRequestDetailsPL(self,key,ImageList):
    request = self.client.factory.create('ns1:CRRequest')
    request['ChangeRequestId'] = key
    #print key
    response = self.client.service.GetChangeRequestById([request])
    for part in response['ChangeRequest']['SoftwareImageRecords']['SoftwareImageRecordEntity']:
      if part['SoftwareImageName'] in ImageList:
        return (part['SoftwareImageName'],part['Status'], \
                part['AssigneeUserName'], part['AnalysisDueDate'] ,\
                part['ReadyDate'],  part['TargetLeadNotes'], part['TargetLeadText'],\
                part['TestLeadNotes'] )
