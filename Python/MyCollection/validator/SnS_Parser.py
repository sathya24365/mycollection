	...
	with open(srcfile2) as file4:
		for line4 in file4:
			...
			if (max == 10):			
				...
				####################################################################
				anr = os.path.join(dstdir,'traces.txt')
				
				with open(anr, 'r') as file1:
					for line in file1.readlines(5):
						if 'pid' in line:
							tanr_pid=line.split()
							anr_pid=tanr_pid[2]							
				print "PID in traces.txt is anr_pid" , anr_pid
				
				logcat1 = os.path.join(dstdir,'logcat.txt')
				with open('input2.txt', 'r') as file1:
					for line1 in file1:
						with open(logcat1, 'r') as file2:
							for line2 in file2:	
								if line1 in line2:
				...
				with open(("Logcat_" + date_string + ".txt"),'r') as logcat2:
					for line in logcat2:
						tcpu=line.split()
						tpid1=line.split("/")
						tpid2=pid1[0].split()
						...
						if (pid == anr_pid):
							dstdir="C:\RAMDUMPS"+"\\"+"VANR"+"\\"+fields[2]+"\\"+fields[3]+"\\"+str(count)+"_"+filename+"\\"
							if not os.path.exists(dstdir):
								os.makedirs(dstdir)
							f4 = open((dstdir+"Scenario.txt"), "a")
							f4.write(tc[getdata()]);
